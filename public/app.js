...
const syncResultsList = document.getElementById('syncResultsList');
...
function resetSyncState() {
  btnSyncCheck.disabled = false;
  btnSyncCheck.innerHTML = '<span class="material-symbols-rounded">fact_check</span> ตรวจสอบอัปเดต';
  syncProgress.style.display = 'none';
  syncSummary.style.display = 'none';
  syncResultsList.style.display = 'none';
  syncResultsList.innerHTML = '';
  btnSyncApply.style.display = 'none';
  syncUpdates = [];
...
btnSyncCheck.addEventListener('click', startSyncCheck);

function startSyncCheck() {
  btnSyncCheck.disabled = true;
  btnSyncCheck.innerHTML = '<span class="material-symbols-rounded spinning">progress_activity</span> กำลังตรวจสอบ...';
  ...
  syncEventSource = new EventSource('/api/sync-check');
  syncEventSource.onmessage = (event) => {
    ...
    if (data.type === 'progress') {
      ...
      syncProgressText.textContent = `กำลังตรวจสอบ ${data.current}/${data.total}... ${data.code} ${data.name}`;
    }

    if (data.type === 'result') {
      results.push(data);
      // Only show non-ok items in the list
      if (data.status !== 'ok') {
        addSyncResultItem(data);
      }
    }
    ...
    if (data.type === 'done') {
      syncEventSource.close();
      syncEventSource = null;
      syncProgressText.textContent = 'ตรวจสอบเสร็จสิ้น';
      syncProgressFill.style.width = '100%';
      btnSyncCheck.innerHTML = '<span class="material-symbols-rounded">fact_check</span> ตรวจสอบอีกครั้ง';
      btnSyncCheck.disabled = false;
      ...
      syncSummary.innerHTML = `
        <div class="sync-summary">
          <div class="sync-summary-item sync-sum-ok">
            <span class="sync-sum-num">${data.ok}</span> ใช้ได้
          </div>
          <div class="sync-summary-item sync-sum-update">
            <span class="sync-sum-num">${data.update + data.fixable}</span> อัปเดต
          </div>
          <div class="sync-summary-item sync-sum-broken">
            <span class="sync-sum-num">${data.broken}</span> ไม่มี PDF
          </div>
        </div>
      `;

      // Show apply button if there are updates
      const updatableCount = syncUpdates.length;
      if (updatableCount > 0) {
        document.getElementById('btnSyncApplyText').textContent = `อัปเดต ${updatableCount} รายการ`;
        btnSyncApply.style.display = 'flex';
      }
    }
  };
  ...
  syncEventSource.onerror = () => {
    syncEventSource.close();
    syncEventSource = null;
    syncProgressText.textContent = 'เกิดข้อผิดพลาด กรุณาลองใหม่';
    btnSyncCheck.disabled = false;
    btnSyncCheck.innerHTML = '<span class="material-symbols-rounded">fact_check</span> ลองใหม่';
  };
}

function addSyncResultItem(data) {
  syncResultsList.style.display = 'block';

  const iconMap = {
    'update': { cls: 'sync-icon-update', icon: 'arrow_upward' },
    'broken-fixable': { cls: 'sync-icon-fixable', icon: 'build' },
    'broken': { cls: 'sync-icon-broken', icon: 'error' }
  };
  const { cls, icon } = iconMap[data.status] || iconMap['broken'];

  const hasUpdate = data.latestPdf;
  if (hasUpdate) {
    syncUpdates.push({ code: data.code, newPdf: data.latestPdf });
  }

  const idx = hasUpdate ? syncUpdates.length - 1 : -1;

  const div = document.createElement('div');
  div.className = 'sync-result-item';
  div.innerHTML = `
    <div class="sync-result-icon ${cls}">
      <span class="material-symbols-rounded">${icon}</span>
    </div>
    <div class="sync-result-info">
      <div><span class="sync-result-code">${data.code}</span> <span class="sync-result-name">${data.name}</span></div>
      <div class="sync-result-detail">
        ${data.currentPdf}
        ${hasUpdate ? `<span class="sync-arrow">→</span> ${data.latestPdf}` : ' (ไม่พบ PDF)'}
      </div>
    </div>
    ${hasUpdate ? `<div class="sync-result-check"><input type="checkbox" checked data-idx="${idx}"></div>` : ''}
  `;
  syncResultsList.appendChild(div);
}

btnSyncApply.addEventListener('click', async () => {
  // Collect checked updates
  const checks = syncResultsList.querySelectorAll('input[type="checkbox"]');
  const selected = [];
  checks.forEach(cb => {
    if (cb.checked) {
      const idx = parseInt(cb.dataset.idx);
      if (idx >= 0 && syncUpdates[idx]) selected.push(syncUpdates[idx]);
    }
  });
  ...
  const origHTML = btnSyncApply.innerHTML;
  btnSyncApply.innerHTML = '<span class="material-symbols-rounded spinning">progress_activity</span> กำลังอัปเดต...';
  ...
  try {
    ...
    if (result.success) {
      btnSyncApply.innerHTML = `<span class="material-symbols-rounded">check_circle</span> อัปเดตสำเร็จ ${result.updated} รายการ`;
      btnSyncApply.style.background = 'linear-gradient(135deg, #059669, #10b981)';
      loadStats();
    ...
    alert('เกิดข้อผิดพลาด: ' + e.message);
    btnSyncApply.innerHTML = origHTML;
